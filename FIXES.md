...
## F18. Матроска-контейнер (MKV/WEBM) направляется в DirectShow — Pro.mkv не воспроизводился

> ✅ **Внедрено 2026-08-16.** Пересобраны x86+x64, переупакованы .wlx/.wlx64, test_verify T01–T06 PASS.

### Проблема
`Pro.mkv` (H.264 видео-трек, **без аудио**, 15 МБ, ~40 с) в TC не воспроизводился: окно открывается, статус «Playing», но ни картинки, ни звука — мёртвый плеер. Другие плееры в системе играют.

### Диагностика (харнесс с реальными функциями плагина + message pump)

| Файл (контейнер/кодеки) | MF (MFPlayer) | DS (VMR-9/LAV) |
|---|---|---|
| 1.mp4, 5.mp4 (MP4, AAC+H.264) | ✅ играет | ✅ |
| 2/3/4/7.mp4 (MP4, H.264 **без аудио**) | ✅ играет | ✅ |
| 6.avi (AVI, VP9) | ✅ играет | ✅ |
| **Pro.mkv (MKV, H.264 без аудио)** | ❌ **Open/Play S_OK, но часы стоят: pos=0.00, dur=40.33 известна** | ✅ играет (pos растёт) |
| 6ix9ine.webm (WebM, AV1+Opus) | ❌ async-ошибка (isPlaying=0, dur=0) | ✅ играет |

**Корень (два разных механизма отказа MF, общий триггер — контейнер Matroska):**
1. **Pro.mkv — тихий застой.** `MFPlayer_Open`/`MFPlayer_Play` возвращают S_OK, media item создаётся (длительность 40.33 с известна), ошибка-событие не приходит, но позиция навсегда 0.00. Ни C2 (асинхронные ошибки), ни DS-fallback по отказу Open не срабатывают — плагин молча висит на «Playing».
2. **6ix9ine.webm — громкая async-ошибка** (media item не создаётся; ловится C2). Ранее (F17) атрибутировалось AV1, но полная матрица показала: **видео-only MP4 (2/3/4/7.mp4) через MF играют, тот же кодек в MKV — стопор** → общий фактор — контейнер, а не кодек. Проверка AV1 оставлена как страховка для AV1-в-MP4 (не наблюдался).
...
## F19. Клавиши-шорткаты после переключения файла (пользовательский отчёт)

> ✅ **Внедрено 2026-08-17.** Пересобраны x86+x64, переупакованы .wlx/.wlx64, test_verify T01–T06 PASS.

### Проблема
Space (пауза), M (mute), S (stop) и стрелки работают после F3 на первом видео, но **перестают после перехода/переключения на следующий файл** (n/p, авто-переход по концу трека). Esc при этом работает всегда и всегда закрывает окно — поведение пользователь просил не менять.

### Диагноз
Клавиши обрабатывает `cbNewMain` (`WM_KEYDOWN`, Defect #16), но сообщения до него доходят только когда фокус на видео-окне (его субкласс `VideoWndProc` пересылает `WM_KEYDOWN` в `hMainWnd`) или на самом главном окне. Целевое состояние задаёт фикс Defect #21: `WM_SETFOCUS` главного окна переводит фокус на плейлист (если показан) или на видео-окно.

Каждое переключение (`PlayIndex`, `ListLoadNextW`, конец трека) вызывает `RecreateVideoWindow`: `DestroyWindow` уничтожает **сфокусированное** видео-окно, новое окно создаётся без фокуса, и Windows уводит фокус на другое окно (тулбар/трекбары/окно TC). Никто фокус не возвращает → шорткаты мертвы. DS→DS-переключения окно не пересоздают — там баг не проявлялся (соответствует наблюдению «переход или переключение»).

### Решение
`RestoreFocus(state)` в хвостах `PlayIndex` и `ListLoadNextW` — **после** обновления `state->showPlaylist` (внутри `RecreateVideoWindow` он ещё не актуален):
- аудио/плейлист показан → фокус на плейлист (там свои клавиши + Esc→закрытие через `PlaylistProc`);
- видео → фокус на видео-окно;
- **QuickView пропускается** (`IsQuickView` guard) — не красть фокус у панели файлов TC;
- Esc, колесо мыши, фуллскрин **не тронуты** — фикс возвращает состояние «после переключения» к состоянию «сразу после старта», в котором у пользователя уже работают и клавиши, и Esc.

**[TC-тест]** — после переключения (n/p, авто-переход): Space/M/S/стрелки работают; Esc закрывает окно как раньше; колесо — громкость/перемотка; плейлист в аудио-режиме; фуллскрин (Esc → выход).

> ✅ **Подтверждено пользователем в TC (2026-08-17):** клавиши работают после переключений как задумано; Esc и остальное поведение без изменений.
