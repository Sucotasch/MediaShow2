...
> **Статус (2026-08-09):** фиксы внедрены по FIXES.md (F1–F6, F8–F15, F7 фаза 1). Сборка чистая, все python-тесты проходят, быстрый тест в TC — видимых багов нет. Отложено: F7 фаза 2. Подробности — в FIXES.md «Статус внедрения».
...
- **F7 фаза 2 (ListLoadNextW):** отложенное пересоздание окна (`IDT_RECREATE`, 300 мс) остаётся — немедленный `RecreateVideoWindow` может вернуть зависание на быстрых переключениях. Требует ручного прогона сценария 6.avi→7.mp4→5.mp4 в реальном TC.
