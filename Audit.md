...
Дата: 2026-08-09. Объём: ~3000 строк C++ (dllmain.cpp + mf_player + ds_player), C++17, CMake, Windows SDK. Ревью выполнено по ReviewPrompt.txt: целостно, с запуском валидаций и готовыми фиксами.

> **Статус (2026-08-09):** фиксы внедрены по FIXES.md (F1–F15, включая F7 фазы 1 и 2). Сборка чистая, все python-тесты проходят, быстрый тест в TC — видимых багов нет. F7 фаза 2 ждёт контрольного прогона быстрых ↓/↑ в TC. Подробности — в FIXES.md «Статус внедрения».

---

## 0. Выполненные валидации

| Проверка | Результат (исходно) | Результат (после фиксов) |
|---|---|---|
| `cmake --build build-x64 --config Release --clean-first` | ✅ 0 ошибок, 0 предупреждений | ✅ 0 ошибок, 0 предупреждений |
| `python test_parse.py` | ❌ 2/8 кейсов падают (edge-кейсы малых размеров) | ✅ ALL TESTS PASSED (F15) |
| `python test_parse2.py` | ⚠️ показывает НЕВЕРНЫЙ результат для `05 - Shame as a Weapon.mp3 13 747 831` → имя остаётся с размером | ⚠️ устарел — отражает старый 3-групповой алгоритм; после F1 неактуален |
| `python test_parse3.py`, `test_parse_c.py`, `test_debug.py` | ✅ проходят | ✅ проходят |
| `python test_bug.py` | ❌ **крашится** `IndexError: list assignment index out of range` (строка 18) | ✅ ALL TESTS PASSED (F15) |
| `python test_verify.py` | ❌ **крашится** `FileNotFoundError` — хардкод старого пути `...MediaShow_v0.9.5_patched\MediaShow2` | ✅ T01–T06 PASS (путь фиксирован на `build-x64`) |
| `python test_tb2.cpp / test_trackbar.cpp` | ⚠️ это C++-файлы, python их не запускает (ожидаемо) | ⚠️ то же |
| `python package.py` | ✅ создаёт `.wlx`/`.wlx64` | ✅ создаёт `.wlx`/`.wlx64` |
| Ручной тест в TC (F3/QuickView, VP9→H.264) | — не выполнялся (headless) | ✅ видимых багов нет |

Вывод: исходно сборка здорова, но 3 из 7 тест-скриптов сломаны или вводят в заблуждение (см. §5). **После внедрения фиксов F15 все тесты проходят, пайплайн end-to-end зелёный.**
...
- **F7 фаза 2 (ListLoadNextW):** внедрена — немедленный `RecreateVideoWindow`, отложенный `IDT_RECREATE` удалён. Риск исторического хэнга на быстрых ↓/↑ остаётся; контрольный прогон сценария 6.avi→7.mp4→5.mp4 + серия быстрых ↓/↑ в реальном TC, при проблеме — откат к `79b7220`.
- **F6 (рефкаунт MF):** остаточная узкая гонка «окно закрывается в момент конца трека» — задокументирована, полностью не устранима без глобальной реструктуризации времени жизни PluginState.
- **C8 (вкладки):** фикс внедрён (SameDirectory + очистка hLastPluginWnd в ListCloseWindow); сценарий с двумя табами стоит прогнать отдельно — быстрый тест багов не показал.
- **C11 (CoInitializeEx):** внедрён аккуратно — только успешный вызов, без CoUninitialize (CO_E_ALREADYINITIALIZED — не ошибка).
...
- Ревью по коду и логам `List.txt`/`PROJECT_CONTEXT.md`; ручной запуск в TC на момент ревью не выполнялся (окружение без TC). **После внедрения фиксов быстрый тест в TC проведён пользователем — видимых багов нет.**
