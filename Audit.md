...
## Addendum 2026-08-16: реальный баг — Pro.mkv (Matroska) не воспроизводился, исправлено (F18)

Пользовательский отчёт: `Pro.mkv` (H.264, **без аудио**, 15 МБ) не воспроизводится плагином, другие плееры играют. Диагностика — харнесс с реальными функциями плагина. **Важно про харнесс:** без message pump'а MF-часы не идут вообще (ложный «стопор» для всех файлов, включая заведомо рабочие); pump обязателен.

Полная матрица (10 файлов):

| Файл (контейнер/кодеки) | MF | DS |
|---|---|---|
| 1.mp4, 5.mp4 (MP4, AAC+H.264) | ✅ играет | ✅ |
| 2/3/4/7.mp4 (MP4, H.264 без аудио) | ✅ играет | ✅ |
| 6.avi (AVI, VP9) | ✅ играет | ✅ |
| **Pro.mkv (MKV, H.264 без аудио)** | ❌ **Open/Play S_OK, media item создан (dur=40.33), НО часы стоят pos=0.00 — тихий застой, событие не приходит** | ✅ играет |
| 6ix9ine.webm (WebM, AV1+Opus) | ❌ async-ошибка (isPlaying=0, dur=0) — ловится C2 | ✅ играет |

**Корень:** общий триггер — **контейнер Matroska**, не кодек. Видео-only MP4 с тем же H.264 через MF играют; тот же H.264 в MKV — тихий застой. У Pro.mkv `MFPlayer_Open` возвращает S_OK (не ошибку!), поэтому DS-fallback («вторая попытка») не срабатывает, C2 не ловит (нет события) — плагин молча висит на «Playing». Атрибуция F17 «AV1-видео» была неполной: webm-файл ломается по той же контейнерной причине (плюс нехватка декодера AV1).

**Фикс (внедрён, пересобран, T01–T06 PASS):** в начало `MFPlayer_NeedsDS` добавлена `MF_IsMatroska()` — чтение первых байтов файла, EBML-магия `1A 45 DF A3` + DocType `matroska`/`webm` → DS. Не-Матроска файлы не затронуты (проверено: 7.mp4/6.avi → 0, webm/Neuropulse.mp4 → 1 как раньше). Проверка AV1 оставлена как страховка для AV1-в-MP4. **[TC-тест]** — F3 на Pro.mkv.

**Новые находки (задокументированы, не игнорированы):**
- **Н4.** Тихий застой MF: `Open`/`Play` S_OK + созданный media item, но нулевой ход часов, без события. C2 не покрывает этот класс — контейнерная маршрутизация закрывает его для Matroska, но класс в целом (любой другой контейнер/файл с таким поведением) остаётся открытым.
- **Н5.** Порядковая зависимость MF в одном процессе: после застойного Matroska-воспроизведения следующий `MFCreateMediaPlayer` не создаёт media item (7.mp4: isPlaying=0, dur=0); поодиночке файлы играют. Для плагина недостижимо (Matroska не попадает в MF), ограничение харнесса — не смешивать Matroska и MF-файлы в одном процессе.
- **Н6.** `DSPlayer_Play` возвращает S_FALSE на первом `Run` **для всех** DS-тестов (не только webm — см. Н1): позиция при этом растёт, вреда нет. Случайное поведение VMR-9/графа, не специфично для файла.
- **Н7.** `MediaShow.ini` (или MediaShow2.ini?) в репозитории не существует — плагин создаёт его при первом запуске; проверить, что регистрация расширений (pluginst.inf `defaultextension` + `ListGetDetectString`) — единственный источник истины для маршрутизации файлов в TC.
- **Н8.** Клавиши-шорткаты (Space/M/S/стрелки) умирали после переключения файла (пользовательский отчёт): `RecreateVideoWindow` уничтожает сфокусированное видео-окно, фокус не восстанавливается, и `WM_KEYDOWN` перестаёт доходить до `cbNewMain` через `VideoWndProc`. Esc не затрагивался (обрабатывается на уровне TC — работает всегда). **Исправлено F19** (2026-08-17): `RestoreFocus()` в хвостах `PlayIndex`/`ListLoadNextW` (showPlaylist-aware, QuickView-guard).
