...
- **2026-08-16 — Matroska-контейнер (Pro.mkv) не воспроизводился — исправлено (F18):** `Pro.mkv` (H.264 без аудио) через MF давал **тихий застой** — `MFPlayer_Open`/`Play` = S_OK, media item создан (dur=40.33), но часы стоят на 0.00, событие не приходит; DS-fallback по отказу Open не срабатывает. Полная матрица из 10 файлов показала: **все MP4/AVI (включая видео-only H.264) через MF играют, оба Matroska-файла (mkv и webm) — нет** → общий фактор — контейнер, а не кодек (уточнение F17). В начало `MFPlayer_NeedsDS` добавлена `MF_IsMatroska()` — EBML-магия `1A 45 DF A3` + DocType `matroska`/`webm` → DirectShow. Не-Матроска файлы не затронуты. Пересобраны x86+x64, переупакованы .wlx/.wlx64, test_verify T01–T06 PASS. ✅ **Подтверждено в TC:** Pro.mkv играет в F3 и QuickView; другие MKV с VP9 тоже играют (пользователь, 2026-08-16).
